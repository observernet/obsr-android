...
import org.obsrj.core.ScriptException;
...
import global.coin.DefaultCoinSelector;
...
import static global.CoinCoreContext.NETWORK_PARAMETERS;

/**
 * Created by mati on 18/04/17.
 */

public class ObsrModuleImp implements ObsrModule {

    private static final Logger logger = LoggerFactory.getLogger(ObsrModuleImp.class);
    ...
    public ObsrModuleImp(ContextWrapper contextWrapper, WalletConfiguration walletConfiguration, ContactsStoreDao contactsStore, RateDbDao rateDb, BackupHelper backupHelper) {
    ...
    public Transaction buildSendTx(String addressBase58, Coin amount,Coin feePerKb, String memo,Address changeAddress) throws InsufficientMoneyException{
        Address address = Address.fromBase58(CoinCoreContext.NETWORK_PARAMETERS, addressBase58);
    ...
    public Transaction completeTxWithCustomFee(Transaction transaction,Coin fee) throws InsufficientMoneyException{
        ...
        walletManager.completeSend(sendRequest);
        // if the fee is different to the custom fee and the tx size is lower than 1kb (1000 bytes in coin core)
    ...
    public int getProtocolVersion() {
        return CoinCoreContext.NETWORK_PARAMETERS.getProtocolVersionNum(NetworkParameters.ProtocolVersion.CURRENT);
    }
        ...
        try {
            for (TransactionOutput transactionOutput : transaction.getOutputs()) {
                ...
                if (script.isSentToAddress() || script.isPayToScriptHash()) {
                    try {
                        address = script.getToAddress(CoinCoreContext.NETWORK_PARAMETERS, true);
    ...
    public ObsrRate getRate(String coin) {
    ...
    public List<InputWrapper> listUnspentWrappers() {
        ...
        for (TransactionOutput transactionOutput : walletManager.listUnspent()) {
            Address address = transactionOutput.getScriptPubKey().getToAddress(CoinCoreContext.NETWORK_PARAMETERS,true);
   ...
   public boolean isSyncWithNode() throws NoPeerConnectedException {
    ...
    @Override
    public List<ObsrRate> listRates() {
        return rateDb.list();
    }
    ...
    public void saveRate(ObsrRate obsrRate){
        rateDb.insertOrUpdateIfExist(obsrRate);
    }

...
public class TxUtils {
    ...
    public static String getAddressOrContact(ObsrModule obsrModule, TransactionWrapper data) {
        String text;
        if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
            ...
            if (addressLabel !=null) {
                ...
                try {
                    text = data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(CoinCoreContext.NETWORK_PARAMETERS, true).toBase58();
                }catch (ScriptException e){
                    text = data.getTransaction().getOutput(1).getScriptPubKey().getToAddress(CoinCoreContext.NETWORK_PARAMETERS,true).toBase58();
                }
            }
        }else {
            text = "Error";

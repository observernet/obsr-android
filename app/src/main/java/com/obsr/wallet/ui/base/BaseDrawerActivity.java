...
import com.obsr.wallet.ui.contacts_activity.ContactsActivity;
import com.obsr.wallet.ui.settings_activity.SettingsActivity;
import com.obsr.wallet.ui.wallet_activity.WalletActivity;

import chain.BlockchainState;

public class BaseDrawerActivity extends CoinActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private BroadcastReceiver walletServiceReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.hasExtra(IntentsConstants.INTENT_BROADCAST_DATA_TYPE)){
                if (intent.getStringExtra(IntentsConstants.INTENT_BROADCAST_DATA_TYPE).equals(
                        IntentsConstants.INTENT_BROADCAST_DATA_BLOCKCHAIN_STATE)) {
                    BlockchainState blockchainStateNew = (BlockchainState) intent.getSerializableExtra(
                            IntentsConstants.INTENT_EXTRA_BLOCKCHAIN_STATE);
                ...
                }else if(intent.getStringExtra(IntentsConstants.INTENT_BROADCAST_DATA_TYPE).equals(
                        IntentsConstants.INTENT_BROADCAST_DATA_PEER_CONNECTED)){
                    checkState();
                    updateBlockchainState();
                }
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        localBroadcastManager.registerReceiver(walletServiceReceiver,new IntentFilter(
                IntentsConstants.ACTION_NOTIFICATION));
    }

    private void checkState(){
        long now = System.currentTimeMillis();
        long lastBlockTime = coinApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+ CoinAppContext.OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = obsrModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (obsrModule.getChainHeight()+10>peerHeight) {
            ...
            if (obsrModule.isAnyPeerConnected()) {
                long peerHeight = obsrModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (obsrModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
...
//        }
    ...
    protected double calculateBlockchainSyncProgress() {
        long nodeHeight = obsrModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (obsrModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }

package com.observer.wallet.rate.db;
...
import com.observer.wallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<ObsrRate> implements RateDbDao<ObsrRate> {
    ...
    @Override
    protected ContentValues buildContent(ObsrRate obj) {
    ...
    @Override
    protected ObsrRate buildFrom(Cursor cursor) {
        ...
        return new ObsrRate(coin,value,timestap);
    }
    ...
    public void insertOrUpdateIfExist(ObsrRate obsrRate) {
        if (getRate(obsrRate.getCode())==null){
            insert(obsrRate);
        }else {
            updateByKey(KEY_COIN, obsrRate.getCode(), obsrRate);
        }
    }
}

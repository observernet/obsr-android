...
import com.observer.wallet.ui.transaction_send_activity.custom.inputs.InputsActivity.INTENT_NO_TOTAL_AMOUNT
import com.observer.wallet.ui.transaction_send_activity.custom.inputs.InputsFragment.INTENT_EXTRA_UNSPENT_WRAPPERS
...
class TransactionIdDetailFragment : BaseFragment(), View.OnClickListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
       ...
       compositeDisposable += Observable.fromCallable {
            obsrModule.getTxWrapper(activity?.intent?.getStringExtra(EXTRA_TX_ID))
                ...
                .observeOn(AndroidSchedulers.mainThread()).subscribe({
                    ...
                    if (intent != null) {
                        if (intent.hasExtra(IS_DETAIL)) {
                            transactionWrapper.transaction = obsrModule.getTx(transactionWrapper.txId)
                            isTxDetail = true
                        } else {
                            transactionWrapper.transaction = Transaction(CoinCoreContext.NETWORK_PARAMETERS, intent.getByteArrayExtra(TX))
        ...
        if (transactionWrapper.isStake) {
            ...
            try {
                ...
                for (input in transactionWrapper.transaction.inputs) {
                    ...
                    inputsSum = inputsSum.plus(obsrModule.getUnspentValue(unspent.hash, unspent.index.toInt()))
                }
            ...
            if (transactionWrapper.outputLabels != null && transactionWrapper.outputLabels.containsKey(transactionOutput.index)) {
                ...
                if (addressLabel != null) {
                    ...
                    //label = addressLabel.getAddresses().get(0);
                        label = transactionOutput.scriptPubKey.getToAddress(CoinCoreContext.NETWORK_PARAMETERS, true).toBase58()
                } else {
                    label = transactionOutput.scriptPubKey.getToAddress(CoinCoreContext.NETWORK_PARAMETERS, true).toBase58()
                }
            } else {
    ...
    private fun setupOutputs(list: List<OutputUtil>) {
        ...
        val listItemListener = object : ListItemListeners<OutputUtil> {
            ...
            override fun onLongItemClickListener(data: OutputUtil, position: Int) {
                if (obsrModule.chechAddress(data.label)) {
    ...
    override fun onClick(v: View) {
        ...
        if (id == R.id.txt_inputs) {
            try {
                ...
                bundle.putSerializable(INTENT_EXTRA_UNSPENT_WRAPPERS, obsrModule.convertFrom(transactionWrapper!!.transaction.inputs) as Serializable)

...
import com.observer.wallet.ui.base.BaseFragment;
import com.observer.wallet.ui.base.tools.adapter.BaseRecyclerAdapter;
import com.observer.wallet.ui.base.tools.adapter.BaseRecyclerViewHolder;
import com.observer.wallet.ui.base.tools.adapter.ListItemListeners;

import org.obsrj.core.Coin;
import org.obsrj.core.Transaction;
import org.obsrj.core.TransactionInput;
import org.obsrj.core.TransactionOutPoint;
...
import static com.observer.wallet.ui.transaction_send_activity.custom.inputs.InputsActivity.INTENT_NO_TOTAL_AMOUNT;
import static com.observer.wallet.ui.transaction_send_activity.custom.inputs.InputsFragment.INTENT_EXTRA_UNSPENT_WRAPPERS;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (intent!=null){
            ...
            if (intent.hasExtra(IS_DETAIL)){
                transactionWrapper.setTransaction(obsrModule.getTx(transactionWrapper.getTxId()));
                isTxDetail = true;
            }else {
                transactionWrapper.setTransaction(new Transaction(CoinCoreContext.NETWORK_PARAMETERS,intent.getByteArrayExtra(TX)));
        ...
        if (transactionWrapper.isStake()){
            ...
            try {
                ...
                for (TransactionInput input : transactionWrapper.getTransaction().getInputs()) {
                    ...
                    inputsSum = inputsSum.plus(obsrModule.getUnspentValue(unspent.getHash(), (int) unspent.getIndex()));
                }
            ...
            if (transactionWrapper.getOutputLabels()!=null && transactionWrapper.getOutputLabels().containsKey(transactionOutput.getIndex())){
                ...
                if (addressLabel !=null) {
                    if (addressLabel.getName() != null) {
                        ...
                        label = transactionOutput.getScriptPubKey().getToAddress(CoinCoreContext.NETWORK_PARAMETERS,true).toBase58();
                }else {
                    label = transactionOutput.getScriptPubKey().getToAddress(CoinCoreContext.NETWORK_PARAMETERS,true).toBase58();
                }
            }else {
                ...
                if (script.isPayToScriptHash() || script.isSentToRawPubKey() || script.isSentToAddress()) {
                    label = script.getToAddress(CoinCoreContext.NETWORK_PARAMETERS, true).toBase58();
    ...
    private void setupOutputs(List<OutputUtil> list) {
        ...
        ListItemListeners<OutputUtil> listItemListener = new ListItemListeners<OutputUtil>() {
            ...
            public void onLongItemClickListener(OutputUtil data, int position) {
                if (obsrModule.chechAddress(data.getLabel())) {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.txt_inputs){
            try {
                ...
                bundle.putSerializable(INTENT_EXTRA_UNSPENT_WRAPPERS, (Serializable) obsrModule.convertFrom(transactionWrapper.getTransaction().getInputs()));

package com.observer.wallet.ui.settings_pincode_activity;
    ...
    public void onKeyClicked(KeyboardFragment.KEYS key) {
        if (key.getValue()<10){
            ...
            if (lastPos==4){
                ...
                if (reason==OLDER_PIN){
                    if (pincode.equals(coinApplication.getAppConf().getPincode())){
                        clear();
                        reason = NEW_PIN;
                    ...
                    coinApplication.getAppConf().savePincode(pincode);
